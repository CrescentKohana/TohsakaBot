        ...
        loop do
          ...
          role_db.each do |k, v|
            ...
            if time_now >= v['time'].to_i + (v['duration'] * 24 * 60 * 60)
              ...
              to_be_removed[[user_id, role_id]] = server_id unless to_be_removed[{user_id => role_id}] == 0
            else
          ...
          to_be_removed.each do |k, v|
            Discordrb::API::Server.remove_member_role("Bot #{AUTH.bot_token}", v, k[0], k[1]) unless v == 0
          end
          sleep(60)
        end
      end
