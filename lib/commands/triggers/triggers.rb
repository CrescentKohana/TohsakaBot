        ...
        sorted.each do |key, value|
          ...
          if (value["user"].to_s =~ author_id) == 0
            ...
            if current_triggers[pos][2].empty?
              output << "#{sprintf("%4s", current_triggers[pos][0])} | #{sprintf("%-21s", current_triggers[pos][1][0..15])} | #{current_triggers[pos][3][9..-1][0..20]}\n"
            else
              output << "#{sprintf("%4s", current_triggers[pos][0])} | #{sprintf("%-21s", current_triggers[pos][1][0..15])} | #{current_triggers[pos][2].lines.first.chomp[0..20]}\n"
            end
            pos += 1
          end
        end
